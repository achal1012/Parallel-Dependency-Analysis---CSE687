  ...
  for(int i=1; i<argc; ++i){
    ...
    try{
      ...
      while(pParser->next())
        pParser->parse();
      std::cout << "\n";
	  Repository* repo = Repository::getInstance(argv[i]);
	  //Retriving Root Element to set its linecount and Complexity.
	  element *rootElem = repo->getAST()->getRoot();
	  rootElem->lineCount = repo->lineCount();
	  rootElem->complexity=repo->getAST()->Complexity(rootElem);
	  //Printing AST on the command prompt
	  repo->getAST()->TreeWalk(repo->getAST()->getRoot());
	  std::cout << "\n\n";
	  Util::title("TypeTable");
